...
- **Per-connection table rules** (`tableRules[profileId] = { mode, patterns }`, mode `all`/`denylist`/`allowlist`): patterns are `table` or `database.table` with `*` wildcards, matched case-insensitively. Enforcement must stay complete on **all four** routes or it is decorative — (1) `assertTableAllowed()` for any tool with a `table` argument, (2) filtering in `list_tables`/`find_tables`/`find_columns` so blocked tables are invisible, (3) `guardSql()` on `run_select`, and (4) `guardSql()` on `execute_sql`. `guardSql` resolves the database's real table list (15s cache, cleared by `clearSessionGrants()`) and rejects the statement if any forbidden name appears as an identifier — it over-blocks a column that shares a blocked table's name, which is the safe direction given there is no SQL parser. Literals are blanked for `run_select` (validated read-only SQL cannot turn a string into an identifier) but **not** for `execute_sql`, where `PREPARE` can. An empty allowlist blocks everything, deliberately.
...
- **Shared code path:** MCP calls the *same* `dbUpdateCell` / `dbInsertRow` / `dbDeleteRow` / `db*Column` functions the UI does, so the `??`/`?` parameterization and `isSafeColumnType()` apply automatically. `db-*` IPC handlers are now thin delegates to these named functions — keep that split. `run_select` adds `isReadOnlySQL()` and a 200-row default / 1000-row cap. The validator strips comments, string literals **and backtick-quoted identifiers** before scanning, then allow-lists the leading keyword and deny-lists mutating ones. Two deliberate rules: a leading `SHOW` skips the deny-list (so `SHOW CREATE TABLE` works — `SHOW` cannot mutate), while `EXPLAIN` stays fully scanned because `EXPLAIN ANALYZE DELETE …` really executes the DML on MySQL 8.0.18+/MariaDB. The function carries a comment listing allowed/blocked examples — keep it current.
- **Tool output is deliberately terse.** `tools/call` returns `JSON.stringify(out)` with **no indent** — pretty-printing put every scalar on its own line and overran client tool-result ceilings (a 1000x3 `run_select` was 5,012 lines). Keep it compact. `list_tables` returns bare names by default and adds `approxRows`/`engine` only under `detail:'full'`; `run_select` reports `totalRows` beside `truncated` so a client never needs a second `count_rows` to interpret the cut.
- **Validate, never coerce.** `requireArgs()` enforces each tool's own `inputSchema.required` — skipping `database` when the profile pins one, since it is implied rather than missing — and `intArg()` rejects non-integers and out-of-range values. Previously a missing `table` reached SQL as the literal string `undefined`, and `limit: 0` silently became 200.
- **`run_select` accepts `params`**, forwarded to mysql2 as `?` bindings, so the read path is parameterized like the write path.
- **Types are reported, not converted.** mysql2 returns DECIMAL/BIGINT as strings; `run_select` adds `columnTypes` and a note rather than casting, because values above 2^53 lose precision as JS numbers. `serializeCell` is shared with the renderer grid — never change its rules to suit an MCP client. `db-query` cols gained an additive `typeName`; the grid reads cols by named field only, so extra fields are safe.
- **Statement timeouts:** `applyStatementTimeout()` tries MariaDB `max_statement_time` (seconds) then MySQL `max_execution_time` (ms), ignoring both failures. MCP reads get 15s, `execute_sql` 60s. Without it, three runaway queries exhausted the `connectionLimit: 3` MCP pool.
- **Schema introspection** (`dbListTablesRich` / `dbTableSchema` / `dbFindTables` / `dbFindColumns` / `dbServerDetail` / `dbSampleRows`) backs the read tools `list_tables`, `describe_table`, `find_tables`, `find_columns`, `server_info`, `sample_rows`. All read-only and parameterized. `describe_table` *composes* `dbDescribeTable` (the same DESCRIBE shape the Variables tab uses) with indexes, FKs and table meta — it does not replace it.
